...
#### GET
Возвращает данные о тексте неделе, список ключей к тексту, статусе отправки проверки, список комментариев к предложениям.
...
```json
{
    ...
    "evaluation": {
        ...
        "sentences_review": [
            {
                "sentence_number": 2,
                "evaluator_comment": "Сложно сказать, почему ключевые особенности структуры проекта освещают чрезвычайно интересные особенности картины в целом, однако конкретные выводы, разумеется, разоблачены",
                "mistake_type": "K08"
            },
            {
                "sentence_number": 14,
