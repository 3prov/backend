    ...
    def _create_evaluations():
        ...
        def _make_random_one(evaluator: User):
            for eval_work in WorkDistributionToEvaluate.objects.filter(
                ...
                for sentence_number in range(eval_work.work.sentences_count):
                    if randint(0, 5) == 0:
                        EssaySentenceReview.objects.create(
                            evaluator=evaluator,
                            sentence_number=sentence_number,
                            essay=eval_work.work,
                            evaluator_comment=choice(_LOREM.split('. ')),
                            mistake_type=choice(list(EssaySentenceReview.MistakesEnum)),
                        )
