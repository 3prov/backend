...
class SortByRatingAlgorithm:
    ...
    def make_optionally_distribution_for_volunteer(
            ...
            users_and_rating_difference.append(
                {
                    ...
                    'existing_evaluations_of_works': existing_evaluations_of_works.filter(
                        work__author=participant
                    ).count(),
                }
            )
        ...
        for participant in sorted_users_and_rating_difference:
            result.append(
                ResultPair(evaluator=volunteer, work_author=participant['participant'])
            )
