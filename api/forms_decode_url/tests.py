...
class ManagementTest(APITestCase):
    ...
    def send_essay_from_user(self, user: User):
        ...
        response = self.client.get(reverse('form_essay_by_encoded_part', args=[encoded_url]))
        data = {
            'body': f'сочинение от {user.username}.'
        }
        response = self.client.post(response.json()['urls']['to_POST'], data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_simulate_evaluation_pass(self):
        ...
        response = self.client.get(reverse('form_evaluation_by_encoded_part', args=[form_eval.url]))
        ...
        for i in range(1, 13):
            self.assertEqual(response.json()['criteria'][f'k{i}'], data['criteria'][f'k{i}'])
        ...
        response = self.client.post(reverse('evaluation_essay_sentence_review_form_url_post', args=[form_eval.url]), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.json()['evaluator_comment'], data['evaluator_comment'])
        ...
        response = self.client.post(reverse('evaluation_essay_sentence_review_form_url_post', args=[form_eval.url]), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json()['detail'], 'Проверка этого предложения уже отправлена.')
        self.assertEqual(EssaySentenceReview.objects.all().count(), 1)
        ...
        response = self.client.put(reverse('evaluation_essay_sentence_review_form_url_edit',
                                            kwargs={
